...
author: "T. Evgeniou - This has been edited by Daniel Khoo"
output:
  html_document:
...
<!-- The process is very useful for consulting! Just load the new data and you are ready to go for initial stages.  -->
...
Different scale! Larger scale of GMAT will dominate! Managerial more important, but do we want this? Maybe not, maybe yes. If you want to reserve difference in scale, then dont normalize, if you want to make them equally informative, then scale it! Note NOT always want to scale!

When will you NOT want mean to be 0 and std dev to 1?  How satisfied was MBA student during studies? VARIABILITY of responses is key! forcing them to 1 will lose information! By standardizing, then means the important variable and unimportant variables are treated as  the same! How satisfied are you with jobs, how satisfied are you with IT services? (less important)
...
```
<!-- This code above changes the data to be a z score -->
...
Here, above 0.5 is already considered quite high, so basicaly can intuitively know that correlations higher than 0.5 / 0.7 should be 'grouped' together.
Top 3 names would be Academic, Bottom 3 about Teamwork/Leadership. Comms is like both?
...
```{r}
UnRotated_Results<-principal(ProjectDataFactor, nfactors=ncol(ProjectDataFactor), rotate="none",score=TRUE) #from psych package.
...
```{r}
Variance_Explained_Table_results<-PCA(ProjectDataFactor, graph=FALSE) #PCA is also principal component packages. just different outputs available
...
```
Component is the 'newly derived factor'. Component 1 means 1st derived feature can get this much of information.
Eigenvalue. Summaries information in a new data with smaller amount of columns. try to keep much of the VARIANCE in the data, by having a new dataset with a smaller set of columns. Eigenvalue is a metric of how much of variability does this new component represent? WE DONT KNOW WHAT COMPONENT 1 or 2 IS at this stage. What is 3.74?

If you add up Eigenvalue total, you get =7 which is number of components. Cum Pct of explained Variance = cumulative(Eigenvalue) / Total(Eigenvalue)
if i have 2 components, we already have 85% of the variability of the original data == 'information'

where you stop is based on your judgement. is 86% enough? then just use 2 components, if not then more. but a few statistical 'rule of thumbs' to decide.
...
```
this plots the eigenvalue on y axis and components on x axis. so stop at 3 here at the elbow where increasing components leads to diminishing results

Another rule says eigenvalue above 1 is good, below is not. basically whose information captures above AVERAGE, cos average for each component is 1.

third way. choose how many cumulative % is needed for the business, and add components until threshold is hit. if you want 90%, choose 3. if you want 85%, choose 2.
...
```
this table gives the LOADINGS/WEIGHTS/COEFFICIENTS on how important this component is. its a linear combination.

rotation tries to rephrase, reorder using math to maximise chances of 1. each new derived feature doesnt use too many old variabels, 2. each variabels IDEALLY used by 1 derived component.
...
```
suppress numbers VISUALLY but NOT eliminate them! just dont show so that human eyes can read this easier. some overlap is fine but not if its all over the place!
...
```{r}
NEW_ProjectData <- round(Rotated_Results$scores[,1:factors_selected,drop=F],2) # drop = FALSE makes sure the dimensions of the object are kept intact
...
```
assign scores to the new dimensions. like what we said previously, 1st compoenet is like academic, 2nd component is like team etc.
