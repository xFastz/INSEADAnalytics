...
<!-- FOR THS ASSIGNMENT: part 1 is dimensionality reduction. -->
<!-- part 2 is clustering. at some point in reduction, produce the components. for example component 1 and 2 and 3 and 4 and 5. -->
<!-- Component 1 loads on qn 9 and qn 26, seems to be about status. so rename it as status. -->
<!-- HINT: as done with part 1, proxy component 1 with only its leading question! ignore everything else just use qn 1.9? so when you do part 2 segmenting variables. can just put question 1.9 as segmentation variable. simplify the life quite a bit. be careful of indexing cos ID is 1st column, 1.9 is the 10th column.( something like this) -->

<!-- for the assignment, need to tweak some lines of codes, distance, number of clusters etc. actual deliverable is to answer questions we see in the RMD itself. -->

<!-- the blanks for the ANSWERS. fill in in text the answers. 6 sets of questions like this. fill in the text after iterating a few times. -->
...
* Yes, for example, questions 13 to 17 are quite highly correlated. They mostly relate to the technology aspects of boating, and how 'geeky' the customers are.
...
* The table shows the Eigenvalue, which is a metric for how much variance in the data is captured if they are reduced to a single component. The sum of the Eigenvalue column will equal to 29, which is the number of variables, or in this case questions.
* The plot shows the eigenvalues on the y axis and the number of components on the x axis. As the number of components increase, there is diminishing returns to the information that each additional component brings, thus the plotted line looks to be decreasing, and not increasing.
* On the plot, we would prefer to see a sharp 'elbow' where after a certain number of components, additional components contribute very little information. This would serve as the 'ideal' number of components to reduce to, of course taking into account the business context.
...
Rotated_Factors<-round(Rotated_Results$loadings,3)
...
* Component 1 has heavy loadings on Q9, 26, 3, ,13, 27, 28, 14, 16, 7,17 and 20.Looking at the question set, they mostly relate to an 'enthusiast' customer segment. This group of customers take pride in owning the best and latest boat, and spend a lot of their free time on activities related to boating. They also seem to be the ones most interested in technology, and the latest and greatest.
* Component 2 has heavy loadings on Q 18, 21, 19, 24. This group of questions seems to identify people who boat as a means of exploring nature or as a social activity. This is quite different from the first component.
* 
* Statistical arguments of using the Eigenvalue method seems to suggest 5 components. If I were to give them a name, it would be Enthusiast, Nature, Brand-conscious, Tinkerers, Price-conscious. From an interpretation point of view, it is relatively easy, however, Components 3, 4, and 5, have relatively lower number of questions that indicate if a customer is in that component. Having only 3 questions while Component 1 has almost 11 questions. From a business perspective, it may not be useful to have so many buckets. The simple ones are the price conscious, and enthusiast segments, where a budget friendly model and a premium model can be introduced. Other than that, it might be hard to customize for other segments. Perhaps if possible a more hardy/rugged model for the nature lovers, and a model more reparable for the tinkerers. However, i would be inclined to reduce the number of components.
*
* Using a 65% variance cutoff, it results in 10 components, which is much more complicated than the 5 above.
*
* Finally, I decided to use a manual number of 3 components. It results in 3 distinct components which i can clearly define as Status, Nature, and Budget.
*
* The biggest lesson is that data science is both a science and an art. Science in the sense that it is very quantitative, with statistics at the core. However, it is also flexible and depending on the situation, parameters need to be tweaked so that the results and output is more informative and actionable to the user.
...
* to recap, Factor 1 represents Status, 2 represents Nature and 3 represents price sensitivity.
* Customer 1 for example has high loadings of Status and Nature, with negative loadings on price sensitivity. This is likely a premium segment customer who has a high willingness to pay provided that the product is premium enough and is able to support his need for status and adventure.
* Customer 10 is the opposite, with high price sensitivity, and negative loadings for status and nature. It is likely that he buys boats for functional and practical purposes, so would appreciate a budget model that serves his basic needs.
*
* I will choose the question that contributes the most to the factors. for Status, the question is  13 (buying the latest and greatest). For Nature, qn 21 (escape everyday life) and for Price, qn 12 (prefer basic boat with no frills)
* 
*
*
*
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(14,22,13) #c(10,19,5,12,3) 
...
This is the code the the clustering!


```{r}
Hierarchical_Cluster_distances <- dist(ProjectData_segment, method=distance_used) #computing the distances. distance_used is found in beginning of part 2, so it was already defined as euclidean earlier on. other choises are max, manhataan, canberra etc or can use own function.
Hierarchical_Cluster <- hclust(Hierarchical_Cluster_distances, method=hclust_method) # this is also found in the definitions. was set to ward.D. distance between 2 clusters of customers, not individual customers. ward.D, ward.D2 etc. can look them up. single is minimum, compelte is maximum. ward.D is choose merging that minimised cluster of variances. works quite well. google them if you want
...
```
this is the HEIGHT PLOT. similar to dendrogram but X axis is number of components/clusters. and traverse from RIGHT to LEFT. RIGHT is the leaves basically, the non clusters. start with leaves level, start climbing up the tree by moving left. height plot shows at what height you are at any given time. at some point, there may be a point where you see an ELBOW. in this case is about 5. stop being flat and become steep! this is the point where from cluster 5 to cluster 4, there is a big jump! means putting a very different clusters into the same bucket. Elbow is an indicator of when to stop! but then again, based on business intuition, if you can make 10 products, maybe 10 clusters is ok!
...
cluster_ids_hclust=unique(cluster_memberships_hclust) #cut tree into number of clusters. this has been decided earlier, set to 7.
...
K MEANS code below. k means is different from hierarchical. iterative testing and a bit of randomness.

while this is the segment membership if we use k-means:

```{r}
kmeans_clusters <- kmeans(ProjectData_segment,centers= numb_clusters_used, iter.max=5000, algorithm=kmeans_method) 
# users need to enter early on what is the numb_clusters_used. want 5 or 12 must do early!. OK to do statistical rule by dendrogram, but also biz intuition. then just test a few times iteratively.
...
```
The columns are the AVERAGES per segment per variable. IF across a variable, if segments are different, means truly different. This is used to evaluate if segments are really different in real life or not, does it have meaning?
...
```
plotting the means across indexes. each color is a segment. x axis is different profiling variables. you choose what to have or not to have. but see how orange curve is different from purple curve! very different people.
...
* They indicate how different the average responses of a segment is compared to the average of the entire population test data. Big absolute numbers are more informative, as it shows a difference in attitudes across the segments. Numbers close to 0 means for those questions, the segments don't really explain if there are differences in those attitudes.
* For question 12 for example, Seg 1 has -0.23, Seg 2 has 0.28 and Seg3 has 0.06. This was a question about having a boat that is no frills. In this segmentation, segment 2 heavily likes this budget option, but segment 1, which is the 'premium' segment dislikes having no frills boats.
*
*
*
*
...
This is the last step. we know 2 methods now. if have valid segmentation, across different methods it should not be DRASTICALLY different. want to iterate. try different segmentation variables if another thing makes sense. try with diff number of clusters. try to explain and interpret clusters.
...
* I ultimately selected 3 final segments. This was quite similar intuition to the 3 dimensions to reduce to in Part 1. Essentially, this was driven by business context where a cluster would influence the type and range of products. They should be sufficiently different to attract different type of customers, but not so many products that it would seem to be a customized product for each individual. Having 3 clusters results in easily interpretable segments that we can define as Premium, Nature and Budget.
* From a statistical point of view, using the dendrogram, a cut of the tree at 3 segments seems to be reasonable. They are mostly differentiated enough at the very top from 3 branches. Once we go below to 4 branches or even 5, the differences in the segment are not as obvious.
*
* I iterated the whole process a few times, tweaking the main parameters of number of clusters as well as distance. This was done to see if there were any additional clusters that would be useful from a business context.
*
* To answer the case questions, I would accept the cluster of 3 customer segments, and accordingly create 3 products that would be well suited for those segments. For the premium segment, i would create a most advanced product with top of the line features. This would be charged at a very high price. For the budget conscious, a basic functional no frills boat could be offered. For the Nature segment, i would create an offering that is more suited for hardy reliable usage.
...
# Please use numbers, not column names! e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
independent_variables= c(54:80) # use 54-80 for boats #only question 16 on wards
...
* Profit/Cost values can be determined based on the profit margins for example. If you correctly predict that a customer will buy and they do indeed buy, the profit would be what you would have earned from that transaction. However, if you predict that the customer will buy and the customer does NOT buy, then there is lost cost in trying to acquire the customer.  
*
* The profit matrix certainly relates to final business decisions. Most businesses prioritize maximizing profits, so the model that maximizes expected profits will be what the company follows in its classifications.
*
* The probability threshold determines what level of probability does it take for a customer to be classified/predicted as purchase or not purchase. It affects business decisions because if you set threshold to very high, very little customers would be predicted to buy, and a lot of customers will be predicted to not buy. This could be useful if it is a very highly tailored market where a lot of marketing resources needs to be spent to acquire a single customer. In this case, focusing on the highest chances of customers makes sense. On the other hand, if it is a more generic product, excluding a lot of customers from marketing activities does not seem to help with profits.
*
*
*
*
...
```{r}
# Let's first calculate all probabilities for the estimation, validation, and test data
