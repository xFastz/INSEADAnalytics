...
author: "T. Evgeniou, edited by Daniel Khoo in class!"
output:
  html_document:
...
This is hte code the the clustering!
...
```
The columns are the AVERAGES per segment per variable. IF across a variable, if segments are different, means truly different. This is used to evalute if segmetns are really different in real life or not, does it have meaning?
...
THis is the last step. we know 2 methods now. if have valid segmentation, across different methods it should not be DRASTICALLY different. want to iterate. try different segementation variabels if another thing makes sense. try with diff number of clusters. try to explain and interpret clusters.
